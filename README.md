...
2. **Restore the project library and install R dependencies**:
   ...
   This restores the versions pinned in `renv.lock`, including `resultcheck`, which is used to snapshot intermediate results and keep the example outputs reproducible.
...
4. Run `testthat::test_dir("tests/testthat")` to verify the `resultcheck` snapshots and output files.
5. Render `manuscript/manuscript.qmd` or `slides/slides.qmd` to reproduce the example manuscript or slides.
